...
# Endpoints:
#   POST /turn  -> Analiza la frase (LLM), detecta errores, extrae slots y
#                   responde con frase ATC determinista
...
TEMPLATES = {
    "superficie": {
        ...
        "rodaje": (
            "{indicativo} recibido, pista en uso {pista}, ruede {ruta} al punto de "
            "espera pista {pista}, mantenga posición corta."
        ),
    ...
    "torre": {
        ...
        "despegue_autorizado": (
            "{indicativo}, viento {viento_dir} grados {viento_vel} nudos, autorizado "
            "para despegar pista {pista}, mantenga {instruccion_post}."
        ),
        "transferir_coco_app": (
            "{indicativo} recibido, contacte COCO Aproximación {frecuencia}."
        ),
    },
    "coco_app": {
        "ack": (
            "{indicativo} COCO Aproximación, contacto radar, continúe su ascenso "
            "para {nivel_ft} pies, QNH {qnh}, llame ingresando a la zona {zona}."
        )
    },
    "coco_radio": {
        "ack": (
            "{indicativo} recibido, QNE dos nueve nueve dos, sin tránsito notificado, "
            "notifiquen {minutos} minutos de vuelo normal."
        )
    },
}
...
def extract_callsign(s: str) -> Optional[str]:
    ...
    for w in words:
        ...
        elif re.fullmatch(r"[a-zA-Z]{3,5}", w):
            return w.upper()
    if 2 <= len(letters) <= 6:
        return "".join(letters)
    return None
...
def extract_runway(s: str) -> Optional[str]:
    if "pista 10" in s or "pista uno cero" in s or re.search(r"\b10\b", s):
        return "10"
    if "pista 28" in s or "pista veintiocho" in s or re.search(r"\b28\b", s):
        return "28"
    return None
...
def extract_zone(s: str) -> Optional[str]:
    if "zona eco" in s or "zona echo" in s:
        return "ECO"
    if "zona whisky" in s or "zona whiskey" in s:
        return "WHISKY"
    return None
...
def extract_qnh(s: str) -> Optional[str]:
    ...
    if m:
        return f"{m.group(1)}"
    if "tres cero cero tres" in s:
        return "3003"
    if "tres cero cero cinco" in s:
        return "3005"
    if "tres cero cero seis" in s:
        return "3006"
    return None
...
def format_atc_number(val: Union[int, str]) -> str:
    ...
    Reglas:
    - Pistas y QNH → dígitos individuales ("28" → "dos ocho";
      "3006" → "tres cero cero seis").
    - Frecuencias → dígitos separados con "decimal" ("118.3" →
...
def ensure_env(ctx: Contexto) -> Contexto:
    ...
    if ctx.viento_dir is None or ctx.viento_vel is None:
        w = gen_wind()
        ctx.viento_dir = w["dir"]
        ctx.viento_vel = w["vel"]
...
LLM_SYSTEM = (
    ...
    '{ "intent": "<string>", '
    '"slots": {"indicativo": "<str|null>", "pista": "<10|28|null>", '
    '"nivel_ft": <int|null>, "zona": "<ECO|WHISKY|null>", "qnh": "<str|null>"}, '
    '"errores": ["<string>", "..."], '
...
# ---------- Frase ATC ----------
def atc_phrase(
    ctx: Contexto, slots: Dict[str, Optional[str]], intent: str, missing: List[str]
) -> str:
    ...
    pista = (
        ctx.runway_actual
        or slots.get("pista")
        or choose_runway_from_wind(ctx.viento_dir)
    )
    pista_fmt = format_atc_number(pista)
    qnh_fmt = format_atc_number(qnh)

    if missing:
        return TEMPLATES[fase]["falta_dato"].format(
            faltante=" / ".join(missing), indicativo=indicativo
        )

    if fase == "superficie":
        if intent in ("abrir_plan", "solicitar_rodaje"):
            return TEMPLATES["superficie"]["rodaje"].format(
                indicativo=indicativo, pista=pista_fmt, ruta="A2-A"
            )
    ...
    if fase == "torre":
        ...
        return TEMPLATES["torre"]["line_up_wait"].format(
            indicativo=indicativo, pista=pista_fmt
        )
    ...
    if fase == "coco_app":
        ...
        return TEMPLATES["coco_app"]["ack"].format(
            indicativo=indicativo,
            nivel_ft=format_atc_number(nivel),
            qnh=qnh_fmt,
            zona=zona,
        )
...
def turn(in_: TurnIn):
    ...
    fb_rule = (
        "Correcto." if not missing else f"Falta {', '.join(missing)} en su colación."
    )
    feedback = (fb_llm.strip() or fb_rule) + (
        ("" if not errores else " | " + "; ".join(errores))
    )
...
def synthesize_pcm16_neural(
    text: str, voice_id: str, rate: float, pitch: int = 0
) -> np.ndarray:
    ...
    if not pcm_bytes:
        raise RuntimeError("Polly devolvió audio vacío (neural).")
...
def comp_soft(
    x: np.ndarray, sr=16000, thresh_db=-14.0, ratio=3.0, win_ms=10
) -> np.ndarray:
...
def tts_fx(in_: TtsIn):
    ...
    try:
        ...
        return Response(
            status_code=502, content=str(e).encode("utf-8"), media_type="text/plain"
        )
    except Exception as e:
        return Response(
            status_code=502,
            content=f"Polly neural error: {e}".encode("utf-8"),
            media_type="text/plain",
        )
    x = bp_filter(x)
    x = comp_soft(x)
    x = add_hiss(x)
    x = np.concatenate([x, squelch_tail()], axis=0)
    wav = to_wav_bytes(x)
